...
public final class QueryUtils {
   ...
   */
     ...
     * هي الدالة وظيفتا بد استدعاء makeHttpRequest وبعد ما نجيب inputStream ونحولو ل jsonResponse اجا دور نسوي دالة ترجع قايئمة بالزلازل
     */
    public static ArrayList<Earthquake> extractFeatureFromJson(String earthquakeJson) {
        // If the JSON string is empty or null, then return early.
        //اذا jsonresponse  فاضية مافي  داعي سوي قائمة بالزلازل لانو اصلا فاضية
        ...
        try {
            ...
            // 1-Create a JSONObject from the SAMPLE_JSON_RESPONSE string
            //  1-JSONObject baseJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);
            // 2-Create a JSONObject from the JSON response string
    ...
    private static String makeHttpRequest(URL url) throws IOException {
        //من الجيد عمل المتغيرات خاج قوس try وذلك اذاحدث exception فيمكنناالوصول لهذا المتغير حتا مع وججود الخطأ
        String jsonResponse = "";
        //اذا url فاضي لا داعي لعمل طلب وارساله لل سييرفر
        ...
        try {
            // اول اربع اسطر كلها فقط اعداد ل طلب Http
            urlConnection = (HttpURLConnection) url.openConnection();
            urlConnection.setReadTimeout(10000 /* milliseconds */);
            urlConnection.setConnectTimeout(15000 /* milliseconds */);
            //هذهالميثود تعين نيتنا من هذ الطلب وهو "get"اي قراءة بيانات من السيرفر
            urlConnection.setRequestMethod("GET");
            //هذا السطر ينفذ اتصال للشبكة
     ...
     * نحدد   input stream reader لاننا نعرف ان البيانات اللي جيتنامن سيرفر هي نصية
     */
    private static String readFromStream(InputStream inputStream) throws IOException {
        /*string vs string builder
         * string: immutable (can not change once created)
         * string builder :mutable (can change once created)
         * and is better choice if you want building up a long text gradually over several step  */
        StringBuilder output = new StringBuilder();
        if (inputStream != null) {
            //input stream is present a stream of bytes and evry byte is smsall chunk data
            // so we need to read text from inputStream via buffered reader
            InputStreamReader inputStreamReader = new InputStreamReader(inputStream, Charset.forName("UTF-8"));
            BufferedReader reader = new BufferedReader(inputStreamReader);
            //نطلب من reader اللي هو من buffered reader ان يقرالنا سطر واحد من النص ذاتا هي اقصا استطاعة وهي فقط سطر واحد يقاؤا
            String line = reader.readLine();
            while (line != null) {
                // هنا سنقوم باضافة اليطر المقروء عالنص الكبير
                output.append(line);
                // نعاود زنقول اقرا السطر الي بعدو
                line = reader.readLine();
            }
        }
        //بجرد كتابة to strig  معناها اناجمدت النص وماعاد اقدر ضيف اي نص اخر بواسطة append
        return output.toString();
    }
    /**
     * Query the USGS dataset and return a list of {@link Earthquake} objects.
     *this is "puublic" becuase the EarthquakeAsyncTask needs to interact with,
     *  this helper method that ties all the steps together - creating a URL, sending the request,
     *  processing the response
     */
    //هي الدالة شاملة
     /*method that ties all the steps together - creating a URL, sending the request, processing the response.
    ...
    public static List<Earthquake> fetchEarthquakeData(String requestUrl) {
        //we are giving long time to fetch the response.
       /** try {
